var soundCloud = function ( _fft_size ) {
  ...
  function setupPlayer( audioElement ) {
    ...
    if ( !audioContextCheck ) {
      ...
      self.SAMPLE_RATE = audioContext.sampleRate;
        ...
        SC.get( '/tracks/', {
          ...
          if ( tracks.errors ) {
            ...
            this.streamUrl = function () {
              return sound.stream_url + '?client_id=' + this.client_id;
            };
            successCallback();
  ...
  this.getHighsVol = function ( _min, _max ) {
    var min = _min || 0;
    var max = _max || 100;
    var v = map( this.getRMS( this.getMix()
      .highs ), 0, self.peak_volume, min, max );
    return v || 0;
  }
  ...
  this.getMidsVol = function ( _min, _max ) {
    ...
    var max = _max || 100;
    var v = map( this.getRMS( this.getMix()
      .mids ), 0, self.peak_volume, min, max );
    return v || 0;
  }
  ...
  this.getBassVol = function ( _min, _max ) {
    ...
    var max = _max || 100;
    var v = map( this.getRMS( this.getMix()
      .bass ), 0, self.peak_volume, min, max );
    return v || 0;
  }
