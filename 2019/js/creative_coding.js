...
function brightness( r, g, b, min, max ) {
  ...
  return Math.round( map( b, 0, 100, min, max ) );
};
...
p.rect = function(x, y, width, height ){
  this.fillRect( x, y, width, height );
  if(!this.no_stroke) this.strokeRect( x, y, width, height );
}
...
function border( sz, c ) {
  ctx.fillStyle = c;
  ctx.fillRect( 0, 0, w, sz );
  ctx.fillRect( 0, h - sz, w, h );
  ctx.fillRect( 0, 0, sz, h );
  ctx.fillRect( w - sz, 0, w, h );
}
...
function sinOsc( min, max, angle ) {
  ...
  max -= min;
  return min + Math.abs( Math.sin( angle ) * max );
}
...
function randomGrey( _min, _max, _alpha, _stick ) {
  if ( _max == undefined ) {
    _max = _min;
    _min = 0;
  }
  ...
  var stick = _stick || 10;
  return rgb( sticky( randomInt( min, max ), stick ), _alpha );
}
...
var Vector = function ( _x, _y, _z ) {
  ...
  this.add = function ( v ) {
    //console.log('_vector.x', _vector.x);
...
function Grid( _num_items_horiz, _num_items_vert, _grid_w, _grid_h, _startx, _starty ) {
  //if(_num_items_horiz = 0 || _num_items_vert == 0) return;
